<div id="dev001">
                <a title="#dev001"></a>
                <div id="topRow" class="row">
                    <div class="col-md-12" style="margin-top:20px;">
                        <div style="text-align: center"></div>
                    </div>
                    <div class="col-md-12" style="min-height: 15%">
                        <div class="col-md-6 col-md-offset-3" style="margin-top:2%;">
                            <div style="text-align: center">
                            <div class="track catNo"><b>DEV001</b></div>    
                            </div>
                        </div>
                    </div>
                </div>
                <div id="mid" class="row col-md-12">
                    <div class="col-md-6 col-md-offset-3 hidden-xs text-center">
                        <a href="https://devormrecords.bandcamp.com/album/what-world-when"><img class="rectangleRelease" width="400" height="400" src="img/dev001.jpg"></a>
                    </div>
                </div>
                <div id="bottomRow" class="row col-md-12">
                    <div class="col-md-6 col-md-offset-3" style="margin-top:10px;">
                        <div class="track" style="text-align: center">
                            WHAT WORLD WHEN <br>
                            Eigenheimer & Individualism
                        </div>
                    </div>
                </div>
            </div>